<x-app-layout>
    ...
    <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
      <div class="min-w-0">
        <div class="flex items-center gap-2 text-xs font-medium text-[rgb(var(--color-text-secondary))]">
          ...
          <svg class="h-3.5 w-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><path d="M15 18l-6-6 6-6" /></svg>
          <span class="truncate text-[rgb(var(--color-text-primary))]">إضافة مهمة جديدة</span>
        </div>
      ...
      <div class="flex items-center gap-2">
        <a href="{{ route('projects.tasks.index', $project) }}" class="gdfh-btn gdfh-btn-secondary">
          إلغاء
        </a>
      </div>
    </div>
  ...
  <div class="px-4 py-8 sm:px-6 lg:px-8 lg:py-10">
    <div class="mx-auto max-w-3xl">
      ...
      <form method="POST" action="{{ route('projects.tasks.store', $project) }}" class="gdfh-card p-6 sm:p-8 space-y-6">
        @csrf
        ...
        <div>
          ...
          <input id="title" name="title" type="text" value="{{ old('title') }}" required placeholder="أدخل عنوان المهمة" class="gdfh-input">
          @error('title')
        ...
        <div>
          ...
          <textarea id="description" name="description" rows="4" placeholder="تفاصيل المهمة ومتطلباتها..." class="gdfh-input">{{ old('description') }}</textarea>
          @error('description')
          <p class="mt-1 text-xs text-[rgb(var(--color-error))]">{{ $message }}</p>
          @enderror
        </div>
        ...
        <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
          <div>
            ...
            <select id="status" name="status" class="gdfh-input">
              <option value="todo" @selected(old('status', 'todo') === 'todo')>قيد الانتظار</option>
              <option value="in_progress" @selected(old('status') === 'in_progress')>قيد التنفيذ</option>
              <option value="in_review" @selected(old('status') === 'in_review')>قيد المراجعة</option>
        ...
        <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
          ...
          <div>
            ...
            <input id="due_at" name="due_at" type="date" value="{{ old('due_at') }}" class="gdfh-input">
            @error('due_at')
            <p class="mt-1 text-xs text-[rgb(var(--color-error))]">{{ $message }}</p>
            @enderror
          </div>
