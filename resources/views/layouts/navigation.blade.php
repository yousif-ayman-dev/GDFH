...
<aside class="fixed inset-y-0 left-0 z-40 hidden w-72 lg:flex lg:flex-col bg-white text-slate-800 border-r border-slate-200 dark:bg-slate-900 dark:text-slate-100 dark:border-slate-800 shadow-sm transition-colors duration-200">
  ...
  <nav class="flex-1 overflow-y-auto px-4 py-4 space-y-6" aria-label="التنقل الرئيسي">
    ...
    <div class="space-y-1">
      ...
      <a href="{{ route($item['route']) }}"
        ...
        @if ($isActive)
        <span class="absolute inset-y-1 left-0 w-1 rounded-r-full bg-amber-500"></span>
        @endif
