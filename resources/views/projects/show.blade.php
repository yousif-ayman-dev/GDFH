    ...
    <div class="mx-auto max-w-7xl space-y-6">
      ...
      <div class="grid grid-cols-1 gap-6 xl:grid-cols-3">
        ...
        <div class="space-y-6 xl:col-span-2">
          ...
          {{-- Tasks Workspace --}}
          <section class="gdfh-card overflow-hidden" x-data="{
              isGenerating: false,
              confirmModal: false,
              errorMessage: '',
              successMessage: '',
              async executeGeneration() {
                  ...
                  try {
                      ...
                      const res = await fetch('{{ route('projects.ai-generate-tasks', $project) }}', {
                          method: 'POST',
                          headers: {
                      ...
                      if (res.ok && data.success) {
                          this.successMessage = data.message || 'تم تفكيك وتوليد مهام المشروع بالذكاء الاصطناعي بنجاح ✨';
                          setTimeout(() => { window.location.reload(); }, 1200);
                      } else {
                          this.errorMessage = data.message || 'تعذر توليد المهام بالذكاء الاصطناعي حالياً. يرجى المحاولة لاحقاً.';
                      }
                  } catch (e) {
                      this.errorMessage = 'حدث خطأ غير متوقع في الاتصال بالخادم.';
                  } finally {
            ...
            <div class="flex items-center justify-between border-b border-[rgb(var(--color-border))] p-5">
              ...
              @can('create', [App\Models\Task::class, $project])
              <div class="flex items-center gap-2">
                <button type="button" @click="confirmModal = true" class="gdfh-btn text-xs font-bold border border-purple-500/30 bg-purple-500/10 text-purple-600 dark:text-purple-300 hover:bg-purple-500/20 transition flex items-center gap-1.5 shadow-sm">
                  ...
                  <span>التقسيم الذكي بالذكاء الاصطناعي ✨</span>
                </button>
            ...
            <div class="divide-y divide-[rgb(var(--color-border))]">
              ...
              <div class="p-8 text-center space-y-3">
                ...
                <div class="flex items-center justify-center gap-3 mt-3">
                  <button type="button" @click="confirmModal = true" class="gdfh-btn text-xs font-bold border border-purple-500/30 bg-purple-500/10 text-purple-600 dark:text-purple-300 hover:bg-purple-500/20 transition flex items-center gap-1.5 shadow-sm py-2 px-4">
            ...
            {{-- Smart AI Task Breakdown Confirmation & Progress Modal --}}
            <template x-if="confirmModal">
              <div class="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 p-4 backdrop-blur-sm" x-transition>
                <div class="gdfh-card w-full max-w-lg overflow-hidden shadow-2xl p-6 space-y-5" @click.outside="if (!isGenerating) confirmModal = false">
                  <div class="flex items-center gap-3">
                    <div class="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-purple-500/10 text-purple-600 dark:text-purple-300">
                      <svg class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M9.813 15.904L9 18.75l-.813-2.846a4.5 4.5 0 00-3.09-3.09L2.25 12l2.846-.813a4.5 4.5 0 003.09-3.09L9 5.25l.813 2.846a4.5 4.5 0 003.09 3.09L15.75 12l-2.846.813a4.5 4.5 0 00-3.09 3.09z" />
                      </svg>
                    </div>
                    <div>
                      <h3 class="text-base font-bold text-[rgb(var(--color-text-primary))]">التقسيم الذكي بالذكاء الاصطناعي ✨</h3>
                      <p class="text-xs text-[rgb(var(--color-text-secondary))]">تحليل وتفكيك متطلبات المشروع تلقائياً</p>
                    </div>
                  </div>

                  <div class="text-xs leading-relaxed text-[rgb(var(--color-text-secondary))] space-y-2">
                    <p>سيقوم مساعد الذكاء الاصطناعي بتفكيك مشروع <strong class="text-[rgb(var(--color-text-primary))]">"{{ $project->title }}"</strong> وتوليد خطة عمل تكتيكية من 4 إلى 6 مهام عمل مجدولة ومنظمة وفق المواعيد النهائية للمشروع.</p>
                  </div>

                  {{-- Error Alert --}}
                  <template x-if="errorMessage">
                    <div class="rounded-xl border border-red-500/30 bg-red-500/10 p-3.5 text-xs font-semibold text-red-600 dark:text-red-400">
                  ...
                  <template x-if="successMessage">
                    <div class="rounded-xl border border-emerald-500/30 bg-emerald-500/10 p-3.5 text-xs font-semibold text-emerald-600 dark:text-emerald-400">
                      <span x-text="successMessage"></span>
                    </div>
                  </template>

                  <div class="flex items-center justify-end gap-3 pt-2">
                    <button type="button" @click="confirmModal = false" :disabled="isGenerating" class="gdfh-btn gdfh-btn-secondary text-xs font-bold px-4 py-2" id="ai-breakdown-cancel-btn">
                      إلغاء
                    </button>
                    <button type="button" @click="executeGeneration()" :disabled="isGenerating" class="gdfh-btn text-xs font-bold bg-purple-600 hover:bg-purple-700 text-white transition flex items-center gap-2 px-5 py-2 shadow-md disabled:opacity-50 disabled:cursor-not-allowed" id="ai-breakdown-confirm-btn">
                      ...
                      <span x-text="isGenerating ? 'جاري تحليل المتطلبات وتوليد المهام...' : 'تأكيد وتوليد المهام 🚀'"></span>
                    </button>
                  </div>
                </div>
              </div>
              ...
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <select name="rating" required class="w-full rounded-xl border border-[rgb(var(--color-border))] bg-[rgb(var(--color-surface))] p-2.5 text-xs text-[rgb(var(--color-text-primary))]">
                    ...
                    <option value="2">2 نجوم - مقبولة</option>
                    <option value="1">1 نجمة واحدة - ضعيف</option>
                  </select>
                </div>
              </div>
              ...
              <div class="pt-4 space-y-2 text-xs">
                <div class="flex items-center justify-between">
                  ...
                  <span class="font-bold text-amber-500 text-xs flex items-center gap-1">
                    <svg class="h-3.5 w-3.5 text-amber-500 fill-amber-500 inline" viewBox="0 0 20 20" fill="currentColor"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/></svg> {{ $rev->rating }}.0
                  </span>
                </div>
                @if ($rev->comment)
