    ...
    <section class="gdfh-card overflow-hidden grid grid-cols-1 md:grid-cols-4 min-h-[580px] shadow-sm">
      ...
      <div class="md:col-span-3 flex flex-col justify-between bg-[rgb(var(--color-surface))]">
        ...
        <div class="p-4 border-t border-[rgb(var(--color-border))] space-y-3 bg-[rgb(var(--color-surface-soft)/0.15)]">
          ...
          <form method="POST" action="{{ route('ai.conversations.messages.store', $activeConversation) }}" class="flex items-center gap-3" x-data="{ isSending: false }" @submit="if ($refs.promptInput.value.trim()) isSending = true">
            @csrf
            <input x-ref="promptInput" type="text" name="message" required placeholder="اسأل المساعد الذكي عن أي شيء في منصة Tasker، المشاريع، أو النصائح..." @keydown.enter.prevent="if ($refs.promptInput.value.trim() && !isSending) { isSending = true; $el.form.requestSubmit(); }" class="flex-1 rounded-xl border border-[rgb(var(--color-border))] bg-[rgb(var(--color-surface))] p-3 text-xs text-[rgb(var(--color-text-primary))] placeholder:text-[rgb(var(--color-text-secondary))] focus:outline-none focus:ring-2 focus:ring-[rgb(var(--color-copper))] shadow-sm">
            
            <button type="submit" :disabled="isSending" class="gdfh-btn gdfh-btn-brand text-xs py-3 px-6 font-bold shadow-md flex items-center gap-2">
              ...
              <span x-text="isSending ? 'جاري الإرسال...' : 'إرسال'">إرسال</span>
            </button>
          </form>
        </div>
        @endif
